...
func LoadCLI(path string) (*CLIConfig, error) {
	...
	cfg.Server = envStr("BBGRID_SERVER", cfg.Server)
	cfg.Server = strings.TrimRight(cfg.Server, "/")
	cfg.APIKey = envStr("BBGRID_API_KEY", cfg.APIKey)
	cfg.Insecure = envBool("BBGRID_INSECURE", cfg.Insecure)

	if cfg.Server == "" {
		return nil, fmt.Errorf("server is required (set in config file or BBGRID_SERVER env)")
	}
	if cfg.APIKey == "" {
